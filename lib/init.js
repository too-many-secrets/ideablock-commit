#!/usr/bin/env node
const path = require('path')
const fs = require('fs-extra')
const async = require('async')
const chalk = require('chalk')
const f = require('./helpers.js')
const os = require('os')
const figlet = require('figlet')
const fetch = require('node-fetch')
const inquirer = require('inquirer')

const ideablockAPIURL = process.env.IDEABLOCK_API_URL || 'https://app.ideablock.com'
const repoName = path.basename(process.cwd())
const HOOK_SHEBANG = '#!/bin/sh'
const HOOK_COMMAND = 'ideablock-commit run'
const authFilePath = path.join(os.homedir(), '.ideablock', 'auth.json')
const repoSaveDir = path.join(os.homedir(), '.ideablock', 'commits', repoName)
const log = console.log

let jsonAuthContents = {}

const loginQuestions = [
  {
    type: 'input',
    name: 'email',
    message: 'Email: '
  },
  {
    type: 'password',
    name: 'password',
    mask: '*',
    message: 'Password: '
  }
]

function addIgnoreLine (callback) {
  // Append, never truncate. This file belongs to the user's repo: the old
  // `> .gitignore` wiped every existing rule, which at best re-introduced
  // node_modules to their diff and at worst un-ignored a .env that git had
  // been keeping out of the history.
  const ignorePath = path.join(process.cwd(), '.gitignore')
  fs.ensureFile(ignorePath)
    .then(() => fs.readFile(ignorePath, 'utf8'))
    .then((contents) => {
      if (contents.split(/\r?\n/).some((line) => line.trim() === '.ideablock')) return
      const gap = contents.length === 0 || contents.endsWith('\n') ? '' : '\n'
      return fs.appendFile(ignorePath, gap + '.ideablock\n')
    })
    .then(() => callback(null))
    .catch((err) => {
      // Not fatal to setup, but say so rather than leaving them to find out
      // when .ideablock shows up in a commit.
      log(chalk.yellow('\t   Could not update .gitignore: ' + err.message))
      log(chalk.yellow('\t   Add a line reading .ideablock to it yourself.\n'))
      callback(null)
    })
}

// Install the post-commit hook by writing the file, not by shelling out to
// `echo ... >> file`.
//
// The old command was `echo "#!/bin/bash\nideablock-commit run " >> hook`,
// which had two failures.
//
// On Windows it never worked at all: shell.exec routes through cmd.exe, whose
// echo neither interprets \n nor strips the quotes, so the hook was a single
// literal line with no shebang and no runnable command.
//
// Everywhere, `>>` meant running init twice appended a second copy, so every
// commit anchored twice. That used to cost two Bitcoin transactions, because
// the parity digit was random and the backend's dedup could not see the two
// runs as the same commit.
//
// Appending is still right when the user already has a post-commit hook of
// their own — theirs is preserved and our line is added at the end, rather
// than a second shebang being injected into the middle of their script.
function installPostCommitHook (hookPath) {
  return fs.ensureFile(hookPath)
    .then(() => fs.readFile(hookPath, 'utf8'))
    .then((existing) => {
      if (existing.split(/\r?\n/).some((line) => line.trim() === HOOK_COMMAND)) return
      // LF explicitly. Git refuses to run a hook whose shebang line ends in
      // CRLF, including on Windows.
      const body = existing.trim() === ''
        ? HOOK_SHEBANG + '\n' + HOOK_COMMAND + '\n'
        : existing + (existing.endsWith('\n') ? '' : '\n') + HOOK_COMMAND + '\n'
      return fs.writeFile(hookPath, body)
    })
    .then(() => {
      try {
        fs.chmodSync(hookPath, 0o755)
      } catch (err) {
        // No-op on Windows, where the mode is not meaningful. Git for Windows
        // runs hooks through its own shell regardless of the executable bit.
      }
    })
}

function createDotIdeaBlock (callback) {
  fs.ensureFile(path.join(process.cwd(), '.ideablock', 'ideablock.json'))
    .then(() => fs.writeJson(path.join(process.cwd(), '.ideablock', 'ideablock.json'), { on: true }))
    .then(() => installPostCommitHook(path.join(process.cwd(), '.ideablock', 'post-commit')))
    .then(() => callback(null))
    .catch((err) => log(err))
}

const createHook = function (err, results) {
  if (err) log(err)
  installPostCommitHook(path.join(process.cwd(), '.git', 'hooks', 'post-commit'))
    .then(() => {
      log(chalk.green.bold('\n\t✅ Ideablock Commit has been initialized for the git repository in the current directory.\n'))
      process.exit(0)
    })
    .catch((err) => log(err))
}

const BANNER_IDEABLOCK = [
  '     dMP dMMMMb  dMMMMMP .aMMMb  dMMMMb  dMP    .aMMMb  .aMMMb  dMP dMP ',
  '   amr dMP VMP dMP     dMP"dMP dMP"dMP dMP    dMP"dMP dMP"VMP dMP.dMP  ',
  '  dMP dMP dMP dMMMP   dMMMMMP dMMMMK" dMP    dMP dMP dMP     dMMMMK"   ',
  ' dMP dMP.aMP dMP     dMP dMP dMP.aMF dMP    dMP.aMP dMP.aMP dMP"AMF    ',
  'dMP dMMMMP" dMMMMMP dMP dMP dMMMMP" dMMMMMP VMMMP"  VMMMP" dMP dMP     '
]

const BANNER_COMMIT = [
  '   .aMMMb  .aMMMb  dMMMMMMMMb  dMMMMMMMMb  dMP dMMMMMMP ',
  '  dMP"VMP dMP"dMP dMP"dMP"dMP dMP"dMP"dMP amr    dMP    ',
  ' dMP     dMP dMP dMP dMP dMP dMP dMP dMP dMP    dMP     ',
  'dMP.aMP dMP.aMP dMP dMP dMP dMP dMP dMP dMP    dMP      ',
  'VMMMP"  VMMMP" dMP dMP dMP dMP dMP dMP dMP    dMP       '
]

function banner () {
  const gold = chalk.bold.rgb(255, 216, 100)
  log('\n')
  BANNER_IDEABLOCK.forEach((line) => log(gold(line)))
  BANNER_COMMIT.forEach((line) => log(gold(line)))
  log('\n')
  log(chalk.bold.white('Please login with your Ideablock credentials.'))
  log(chalk.gray('(You can sign up at https://ideablock.com)\n'))
}

function authorize () {
  fs.ensureDirSync(repoSaveDir)

  // Reuse an existing credential.
  //
  // A CLI token (ibk_...) has no expiry: it lives until it is revoked, which is
  // the whole point. Anything else is a legacy session token that used to be
  // stored here and stopped working fifteen minutes after login — those are
  // discarded so init mints a proper one.
  const authExists = fs.pathExistsSync(authFilePath)
  if (authExists) {
    try {
      const existing = fs.readJsonSync(authFilePath)
      if (existing.token && String(existing.token).startsWith('ibk_')) {
        jsonAuthContents = existing
        setup()
        return
      }
      if (existing.token) {
        log(chalk.yellow('\n\t⚠️  Upgrading to a CLI token that does not expire. One more login.\n'))
      }
      fs.removeSync(authFilePath)
    } catch (e) {
      fs.removeSync(authFilePath)
    }
  }

  // Prompt for credentials
  inquirer.prompt(loginQuestions)
    .then(answers => {
      // Mint a long-lived, commit-scoped token rather than taking a session.
      // The label records where it came from so a token left on an old machine
      // can be recognised and revoked from Settings.
      fetch(ideablockAPIURL + '/api/cli/token', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: answers.email,
          password: answers.password,
          label: 'ideablock-commit on ' + os.hostname()
        })
      })
        .then(res => {
          if (res.status === 401 || res.status === 403 || res.status === 500) {
            log(chalk.red('\n\t❌ We cannot find an Ideablock account with those credentials.'))
            log(chalk.red('\t   Please visit https://ideablock.com to register.\n'))
            process.exit(0)
          } else if (res.status === 200) {
            res.json()
              .then(obj => {
                // Store token + user info — no raw password ever touches disk
                jsonAuthContents = {
                  token: obj.token,
                  // No expiry: revocation ends it, nothing else.
                  user: {
                    id: obj.user.id,
                    email: obj.user.email,
                    first_name: obj.user.first_name,
                    last_name: obj.user.last_name
                  },
                  cached_at: Math.floor(Date.now() / 1000)
                }
                fs.ensureFile(authFilePath)
                  .then(() => fs.writeJson(authFilePath, jsonAuthContents))
                  .then(() => {
                    log(chalk.green('\n\t✅ Logged in as ' + obj.user.email + '\n'))
                    setup()
                  })
                  .catch((err) => log(err))
              })
              .catch(() => {
                log(chalk.red('\n\t❌ Unexpected response from Ideablock. Please try again.\n'))
                process.exit(0)
              })
          } else {
            log(chalk.red('\n\t❌ Login failed (status ' + res.status + '). Please try again.\n'))
            process.exit(0)
          }
        })
        .catch(err => {
          log(chalk.red('\n\t❌ Could not reach Ideablock API: ' + err.message))
          log(chalk.red('\t   Is the backend running? Set IDEABLOCK_API_URL if needed.\n'))
          process.exit(0)
        })
    })
}

function setup () {
  if (!f.isRepo()) {
    log('\n\t❗ ' + chalk.bold('The present directory is not a git repository.'))
    log('\t   Please initialize a git repository in this directory before invoking ideablock-commit.\n')
  } else if ((f.isRepo() && !f.exists(path.join(process.cwd(), '.ideablock')))) {
    async.series([addIgnoreLine, createDotIdeaBlock], createHook)
  } else if (f.isRepo() && f.isOn()) {
    log(chalk.green.bold('\n\t✅ Ideablock Commit is initialized for the git repository in the current directory.\n'))
  } else if (f.isRepo() && f.isOff()) {
    createHook()
    fs.writeJsonSync(path.join(process.cwd(), '.ideablock', 'ideablock.json'), { on: true })
  }
}

module.exports.init = function () {
  if (!f.authed()) {
    banner()
    authorize()
  } else {
    // Even if auth file exists, verify token hasn't expired
    try {
      const existing = fs.readJsonSync(authFilePath)
      const nowSecs = Math.floor(Date.now() / 1000)
      if (existing.token_expires && existing.token_expires <= nowSecs) {
        log(chalk.yellow('\n\t⚠️  Your Ideablock session has expired. Please log in again.\n'))
        fs.removeSync(authFilePath)
        banner()
        authorize()
        return
      }
    } catch (e) {
      fs.removeSync(authFilePath)
      banner()
      authorize()
      return
    }
    setup()
  }
}
